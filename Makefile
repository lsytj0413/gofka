...
ROOT := github.com/lsytj0413/gofka
...
test:
	@go test -v -race -coverpkg=./... -coverprofile=coverage.out.tmp -gcflags="all=-N -l" ./...
...
.PHONY: mock
mock: $(MOCKGEN) 
	mockgen -source=xapp/locator.go -destination=xapp/locator_mocks_test.go -package=xapp
